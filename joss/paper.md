...
Over the past decade, several tools have been developed to accelerate intracranial electrode localization and labeling. DEETO [@Arnulfo:2015] introduced automated CT-based segmentation of deep electrodes, but it relied on planned entry and target points, limiting robustness when implanted trajectories deviated from the plan. SEEG Assistant [@Narizzano:2017] extended these capabilities within 3D Slicer for trajectory analysis and contact segmentation, but similarly depended on prior trajectory information and functioned primarily as a standalone platform. GARDEL [@Medina-Villalon:2018], released with EpiTools, further advanced automation by localizing sEEG contacts, assigning anatomical labels, and supporting 3D visualization of downstream analyses, yet remained separate from electrophysiology analysis workflows. FASCILE [@Ervin:2021] and LeGUI [@Davis:2021] further reduced manual effort in contact identification, labeling, and anatomical localization, but remained focused primarily on localization-oriented preprocessing. MNE-Python [@Rockhill:2022] later incorporated CT-MR alignment, GUI-based contact marking, template-space warping, and visualization within a broader electrophysiology analysis environment, but localization remained largely user-guided. iEEG-recon’s VoxTool [@Lucas:2023] subsequently introduced a more comprehensive reconstruction pipeline with semi-automatic labeling, CT-MRI registration, atlas-based assignment, and quality control, though it still relied on manual interaction and remained focused on reconstruction. seegloc [@Wong:2024] provided a largely automated pipeline for coregistration, contact detection, trajectory clustering, and atlas-based labeling, but remained centered on localization and preprocessing. Most recently, Chen et al. [@Chen:2026] improved robustness under practical clinical conditions, but remained centered on localization and labeling. As a result, outputs from these tools are still often transferred into separate platforms for visualization and electrophysiological analysis.

The sEEG-suite proposes a distinct scholarly contribution by integrating and extending the GARDEL workflow directly within Brainstorm. It unifies CT-MRI co-registration, skull stripping, automatic contact detection with live visualization, interactive refinement, atlas-based labeling, and direct linkage of contacts to recordings for downstream analysis, all within the same software. The result is an end-to-end workflow that reduces fragmentation and errors caused by the import/export process between standalone tools, enabling users to carry electrode definitions directly into analysis without additional tooling.

# Software Design

sEEG-suite was built to keep the localization workflow within Brainstorm, a dedicated software for the neuroscience community, rather than spreading it across multiple tools. To support this, GARDEL is integrated as a Brainstorm plugin, allowing users to run automated contact detection directly from the Brainstorm interface. This also simplifies installation and updates, without requiring changes to Brainstorm’s core codebase or data structures, and it works across the major operating systems.
