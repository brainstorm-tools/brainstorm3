...
%                 'markerfacecolor','k');
%  marker2 = line('xdata',face(1,:),'ydata',face(2,:),...
...
%     line('parent',ax1,'xdata',xcp(1,2),'ydata',xcp(2,2),'zdata',0,'marker','o','markerfacecolor','r');
% end
...
if isprop(ax, 'x_RenderTransform')
    xform = get(ax,'x_RenderTransform');
    offset = get(ax,'x_RenderOffset');
    scale = get(ax,'x_RenderScale');
% Patch for Matlab 2014b: those properties were removed from the axes
else
    [xform, offset, V, P, W] = rendertransform(ax);
    scale = [1;1;1];
end
