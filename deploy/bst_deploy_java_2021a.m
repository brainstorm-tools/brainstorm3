function bst_deploy_java_2021a(IS_BIN)
% BST_DEPLOY_JAVA - Brainstorm deployment script (including SPM and FieldTrip).
%
% USAGE:  bst_deploy_java_2021a(IS_BIN=0)
%
% INPUTS:
...
if IS_BIN
    ...
    % FieldTrip / SPM
    if IS_FT_SPM
        compilerFile = fullfile(bstDir, 'deploy', 'bst_javabuilder_2021a_spm.prj');
    else
        compilerFile = fullfile(bstDir, 'deploy', 'bst_javabuilder_2021a.prj');
    end
...
%     end
    ...
    % Compiled jar
    compiledJar = fullfile(compilerOutputDir, 'bst_javabuilder_2021a.jar');
    ...
    classFile = fullfile('org', 'brainstorm', 'file', 'SelectMcr2021a.class');
    ...
    if file_exist(classFile)
        ...
        disp(['WARNING: Packaging without the installation runner, you must:' 10 ... 
              ' - Create and compile the Java project brainstorm_run_2021a' 10 ...
              ' - Copy RunCompiled.class to the packaging folder: ' 10 ...
              '   brainstorm3_deploy\R2021a\brainstorm_run\org\brainstorm']);
    end
    ...
    system(['cd "' jarDir '" & "' jdkDir '\bin\jar.exe" cmf manifest.txt "' bstJar '" bst_javabuilder_2021a org com']);
end
