...
function j = MenuCreate(jMenu, fontSize)
    ...
    for iPlug = 1:length(PlugDesc)
        ...
        % Do not create submenus for compiled version
            ...
            j(ij).usage = gui_component('MenuItem', j(ij).menu, [], 'Usage statistics', IconLoader.ICON_EXPLORER, [], @(h,ev)showUsage(Plug.Name), fontSize);
...
str = strsplit(str, '\n');
nUsersTotal = length(str); 
dates = cellfun(@(x)strsplit(x,':'), str,'UniformOutput', 0);
tmp = cellfun(@(x)datetime(x{1},'InputFormat','yyyy-MM-dd'),dates);


mth = year(tmp) + (month(tmp)-1)./12;
[nUpdate,xUpdate] = hist(mth,length(unique(mth)));

fig_report(xUpdate(1:end-1), nUpdate(1:end-1), 0, ...
           [2021, max(xUpdate(1:end-1))], [], ...
           sprintf('Total number of Downloads: %d', nUsersTotal), [], 'Downloads per month', ...
           [100, 0], 0, 'download');
...
function [hFig, hAxes] = fig_report(x, y, isMarkers, XLim, YLim, wTitle, xLabel, yLabel, wPos, isSave, filename)
    ...
    if ~isempty(strfind(filename, 'download'))
        hText = text(2021.25, 3, 2, 'No data available', ...
                     'Color',     [.7 .7 .7], ...
