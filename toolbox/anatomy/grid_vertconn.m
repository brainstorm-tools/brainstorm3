...
connection = unique([I,J], 'rows');
connection = unique([connection; connection(:, [2,1])], 'rows');

% Compute distance for each of those neighbours
distance = sqrt((GridLoc(connection(:,1), 1) - GridLoc(connection(:,2), 1)).^2 + ...
                (GridLoc(connection(:,1), 2) - GridLoc(connection(:,2), 2)).^2 + ...
                (GridLoc(connection(:,1), 3) - GridLoc(connection(:,2), 3)).^2 );
% Delete connections between points that are too far apart
isDistOk = (distance <= distance_max);
connection = connection(isDistOk, :);
if (nargout >= 2)
    distance = distance(isDistOk, :);
end

% Compute connectivity matrix in sparse mode
VertConn = sparse(connection(:, 1), connection(:, 2), ones(size(connection(:,1))), size(GridLoc, 1), size(GridLoc, 1));
