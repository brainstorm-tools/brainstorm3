function [sMriReg, errMsg] = mri_coregister(MriFileSrc, MriFileRef, TransfSrc, TransfRef, isReslice)
% MRI_COREGISTER: Compute the MNI transformation on both input volumes, then register the first on the second.
%
% USAGE:  [MriFileReg, errMsg] = mri_coregister(MriFileSrc, MriFileRef, TransfSrc=[], TransfRef=[], isReslice)
...
if isReslice
    ...
    % OPTION #2: Linear interp, very similar results, much faster
    newCube = uint8(interp3(Y1, X1, Z1, double(sMriSrc.Cube), Xgrid2, Ygrid2, Zgrid2, 'linear', 0));
...
%     end
    ...
    % Use the reference SCS coordinates if possible (ALWAYS TRUE)
    ...
    % Otherwise, use the SCS from the transformed MRI (IGNORED)
    else
        %%%%%%%%%%% INTERESTING CODE BUT NEVER CALLED
    ...
    if isfield(sMriSrc, 'NCS') && isfield(sMriSrc.NCS, 'R') && ~isempty(sMriSrc.NCS.R) && isfield(sMriSrc.NCS, 'T') && ~isempty(sMriSrc.NCS.T)
        ...
        % Equivalent to:
        sMriReg.NCS.R = sMriRef.NCS.R;
        sMriReg.NCS.T = sMriRef.NCS.T;
    end
...
if ~isempty(MriFileSrc)
    ...
    % Update comment
    sMriReg.Comment = strrep(sMriReg.Comment, '_coreg', '');
    if isReslice
        sMriReg.Comment = [sMriReg.Comment, '_coreg_reslice'];
    else
