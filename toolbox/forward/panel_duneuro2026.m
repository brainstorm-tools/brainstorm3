...
function [bstPanelNew, panelName] = CreatePanel(sProcess, sFiles) %#ok<DEFNU>
    ...
    jPanelSrcModel = gui_river([1,1], [0,6,6,6], 'FEM source model');
        ...
        jRadioSrcModelVen = gui_component('radio', jPanelSrcModel, 'br', 'Multipolar Venant (optimal, faster)',              jGroupSrcModel, '', @(h,ev)UpdatePanel(1), []);
        jRadioSrcModelSub = gui_component('radio', jPanelSrcModel, 'br', 'Local Subtraction (recomended, slower)',         jGroupSrcModel, '', @(h,ev)UpdatePanel(1), []);
        jRadioSrcModelPar = gui_component('radio', jPanelSrcModel, 'br', 'Partial Integration (basic, fastest)', jGroupSrcModel, '', @(h,ev)UpdatePanel(1), []);
        switch lower(OPTIONS.SrcModel)
            case 'multipolar_venant',   jRadioSrcModelVen.setSelected(1);
            case 'local_subtraction',   jRadioSrcModelSub.setSelected(1);
...
%  =================================================================================
    ...
    function UpdatePanel(isForced)
        ...
        if isForced
            ...
            % Get old panel
            [bstPanelOld, iPanel] = bst_get('Panel', 'DuneuroOptions2026');
...
function s = GetPanelContents() %#ok<DEFNU>
    ...
    ctrl = bst_get('PanelControls', 'DuneuroOptions2026');
    if isempty(ctrl)
        s = [];
        return; 
    end
