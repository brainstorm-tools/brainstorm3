function [outStruct, replacedFields] = bst_jsondecode(inString, forceBstVersion)
    ...
    %   - only supports number or string arrays
...
%                 value = [value c]; %#ok<*AGROW>
%             elseif state == STATE.READ_FIELD || ...
%                     (state == STATE.READ_VALUE && valType == VAL.LIST_STR)
%                 token(end + 1) = c;
%             elseif state == STATE.START
%                 state = STATE.START_FIELD;
%             elseif state == STATE.START_VALUE
...
%                 token(end + 1) = c;
%             elseif state == STATE.END_FIELD
%                 % Validate field name is appropriate for matlab.
%                 if ~isvarname(token)
%                     iReplaced = iReplaced + 1;
%                     %         field = ['REPLACED_FIELD_', str2double(iReplaced)];
%                     % Rename like Matlab, though 2 keys could have the same modified name.
...
%             elseif state == STATE.READ_VALUE && valType == VAL.LIST_STR
%                 token(end + 1) = c;
%             elseif state == STATE.READ_VALUE && valType == VAL.LIST_NUM
%                 if ~isempty(token)
%                     n = str2double(token);
%                     token = '';
%                     if ~isempty(n)
...
%                 end
%                 state = STATE.READ_VALUE;
%                 if valType == VAL.LIST_NUM || valType == VAL.LIST_STR
%                     token(end + 1) = c;
%                 else
%                     value = [value c];
%                 end
