function varargout = panel_montage_shortcuts(varargin)
% PANEL_MONTAGE_SHORTCUTS: Edit keyboard shortcuts for the Montages.
...
function [bstPanelNew, panelName] = CreatePanel() %#ok<DEFNU>
    ...
    % Create 9 text boxes
    jText = javaArray('javax.swing.JTextField', 26);
    for i = 1:13
        gui_component('label', jPanelShort, 'br', sprintf('Shortcut %c: ', char(uint8('A') + i - 1)));
        jText(i) = gui_component('text', jPanelShort, 'tab hfill', MontageOptions.Shortcuts{i,2}, {java_scaled('dimension', 120,22)});
        gui_component('label', jPanelShort, [], sprintf('Shortcut %c: ', char(uint8('A') + i + 12)));
        jText(i+13) = gui_component('text', jPanelShort, 'tab hfill', MontageOptions.Shortcuts{i+13,2}, {java_scaled('dimension', 120,22)});
    end
    jText(1).setEnabled(0);
...
function SaveShortcuts()
    ...
    ctrl = bst_get('PanelControls', 'MontageShortcuts');
    % Get all the shortcuts
    for i = 2:26
        MontageOptions.Shortcuts{i,2} = strtrim(char(ctrl.jText(i).getText()));
    end
    % Save shortcuts
