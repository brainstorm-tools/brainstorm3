...
for iResFile = 1:length(ResultsFiles)
    ...
    for k = 1:length(sScouts)
        ...
        if (nComponents == 1)
            ...
            isNIRS = ~isempty(strfind(TestTags, 'NIRS')) || ...
                     ~isempty(strfind(TestTags, 'Summed_sensitivities')) || ...
                     ( isfield( GlobalData.DataSet(iDS).Results(iResult), 'DisplayUnits') && ~isempty( strfind(GlobalData.DataSet(iDS).Results(iResult).DisplayUnits, 'mol.l-1')) && ~isempty( strfind(GlobalData.DataSet(iDS).Results(iResult).DisplayUnits, 'OD')) ) || ...
