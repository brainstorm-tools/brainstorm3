...
function OpenSpikeFile(SpikeFile)
    ...
    DataMat.Parent = bst_fullfile(ProtocolInfo.STUDIES, DataMat.Parent);
    DataMat.Name   = bst_fullfile(ProtocolInfo.STUDIES, DataMat.Name);
    for i = 1 : length(DataMat.Spikes)
        DataMat.Spikes(i).Path = bst_fullfile(ProtocolInfo.STUDIES, DataMat.Spikes(i).Path);
    end
