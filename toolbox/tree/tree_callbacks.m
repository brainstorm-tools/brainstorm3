...
methods_str = {...
    ['<HTML><B><U> Brainstorm method:</U></B><BR>' ...
    '&nbsp;&nbsp;&nbsp;| - Standard Laplacian smoothing — fast and simple, may shrink mesh.<BR>' ...
    '&nbsp;&nbsp;&nbsp;| -  Calls the <CODE>tess_smooth.m</CODE> function.'], ...
    ['<HTML><B> iso2mesh method:</B><BR>' ...
    '&nbsp;&nbsp;&nbsp;| - Curvature-aware smoothing that preserves mesh shape.<BR>' ...
    '&nbsp;&nbsp;&nbsp;| - Calls the <CODE>sms</CODE> function with selectable methods.'], ...
    };

% Ask method
selectedMethod = java_dialog('radio', 'Select the smoothing method:', 'Resample surface', [], methods_str, 1);
...
switch (selectedMethod)
    case 1,  MethodId = 'brainstorm smoothing';
        % defaut parameters
        userIteration = 1;
        useSmoothWeighting = 0.5; % scalar smooth weighting parameter (0-1 less-more smoothing)
        isKeepSize = 1;
        % [check smoothsurf of iso2mesh for more information]
        % Ask user for parameters
        res = java_dialog('input', {'Smooth weighting: [0, 1] (0 less, 1 strong)', 'Iteration: ( number of times to apply the smoothing)'}, ...
            'Smooth surface', [], ...
            {sprintf('%1.2f', useSmoothWeighting), sprintf('%d', userIteration)});
        % Read user input
        useSmoothWeighting = str2double(res{1});
        userIteration = str2double(res{2});
 
        % run the sms smoothing
        newTessMat.Vertices = tess_smooth(TessMat.Vertices, useSmoothWeighting, userIteration, TessMat.VertConn, isKeepSize, TessMat.Faces);
        % tag to add in the history file    
        MethodTag = ['Method: ' MethodId ', Iteration: ' res{2} ', Smooth Weighting: ' res{1}];
    case 2,  MethodId = 'iso2mesh smoothing';
        % defaut parameters
        ...
        % tag to add in the history file    
        MethodTag = ['Method: ' MethodId ': ' smoothingMethods{userMethod} ', Iteration: ' res{2} ', Smooth Alpha: ' res{1}];
end
