...
%% ===== POPUP: STUDYSUBJECT =====
            case 'studysubject'
                ...
                if ~isempty(sSubject.Anatomy) && ~strcmpi(sSubject.Name, bst_get('NormalizedSubjectName'))
                    ...
                    if ~isempty(iCtRaw)
                        gui_component('MenuItem', jMenuImplantation, [], 'GARDEL (external)', IconLoader.ICON_SEEG_DEPTH, [], @(h,ev)bst_call(@seeg_contactid_gardel, iSubject));
                    end
                end
                % Export menu (added later)
    ...
    if ~isAtlas
        ...
        if (length(iAnatomy) <= 1) && isCt
            if ~isempty(sSubject.iAnatomy)
                gui_component('MenuItem', jMenu, [], 'SPM: Skull stripping', IconLoader.(volIcon), [], @(h,ev)MriSkullStrip(MriFile, sSubject.Anatomy(sSubject.iAnatomy).FileName, 'SPM'));
                gui_component('MenuItem', jMenu, [], 'BrainSuite: Skull stripping', IconLoader.(volIcon), [], @(h,ev)MriSkullStrip(MriFile, sSubject.Anatomy(sSubject.iAnatomy).FileName, 'BrainSuite'));
            end
        ...
        if isempty(iAnatomy) && iSubject ~=0
            ...
            iCtRaw = find(cellfun(@(x) ~isempty(regexp(x, '_volct_raw', 'match')), {sSubject.Anatomy.FileName})); 
            if ~isempty(iCtRaw)
                gui_component('MenuItem', jMenuImplantation, [], 'GARDEL (external)', IconLoader.ICON_SEEG_DEPTH, 'GARDEL external tool', @(h,ev)bst_call(@seeg_contactid_gardel, iSubject));
            end
