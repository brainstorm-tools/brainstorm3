...
%% ===== POPUP: SUBJECT =====
            case 'subject'
                ...
                if ~bst_get('ReadOnly') && ((iSubject == 0) || ~sSubject.UseDefaultAnat)
                    AddSeparator(jPopup);
                    % === IMPORT ===
                    gui_component('MenuItem', jPopup, [], 'Import anatomy folder', IconLoader.ICON_ANATOMY, [], @(h,ev)import_anatomy(iSubject), []);
                    gui_component('MenuItem', jPopup, [], 'Import MRI', IconLoader.ICON_ANATOMY, [], @(h,ev)import_mri(iSubject, [], [], 1), []);
                    gui_component('MenuItem', jPopup, [], 'Import surfaces', IconLoader.ICON_SURFACE, [], @(h,ev)import_surfaces(iSubject), []);
                    AddSeparator(jPopup);
                    % === USE DEFAULT ===
            ...
            case 'channel'
                ...
                elseif (length(DisplayMod) >= 1)
                    ...
                    if (length(bstNodes) == 1) && ismember(Device, {'Vectorview306', 'CTF', '4D', 'KIT', 'KRISS', 'BabyMEG'})
                        ...
                        if ismember(Device, {'CTF', 'KIT', 'KRISS', '4D', 'BabyMEG'})
...
%% ===== DISPLAY CHANNELS (3D) =====
function [hFig, iDS, iFig] = DisplayChannels(bstNodes, Modality, SurfaceType, is3DElectrodes, isDetails)
    % Parse inputs
