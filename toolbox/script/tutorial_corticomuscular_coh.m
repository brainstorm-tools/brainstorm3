...
% Rename condition: preprocessed
nodeStudy = panel_protocols('SelectStudyNode', iStudy);
node_rename(nodeStudy, 'SubjectCMC_preprocessed');
sStudy = bst_get('Study', nodeStudy.getStudyIndex);
% Select only good trials
sFilesEpochs = sStudy.Data(~[sStudy.Data.BadTrial]);
...
for ix = 1 :  length(sourceTypes)
    ...
    baseComment = tmp{1};
    newComment = [baseComment, ': ', src_channel, ' ', sourceType];
    % Process: Set name
