...
%         nSamples = diff(sFile.prop.samples) + 1; % This is single epoch samples if epoched.
    ...
    for iHeadSample = 1:nHeadSamples
      ...
      TransfMat = LocationTransform( ... % This function was copied from process_adjust_head_position.m
...
function [SIn, SOut, LIn, LOut] = SphericalBasis(LIn, LOut, Loc, Orient, ...
  ...
  while l <= max(LIn, LOut) 
    ...
    if l <= LIn
      ...
      SIn(:, iS:iS+2*l) = bsxfun( @times, -1./r.^(l+2), ...
    ...
    if l <= LOut
      ...
      %       ST = dYdt;
      % Project along sensor orientations.
      SOut(:, iS:iS+2*l) = bsxfun( @times, -r.^(l-1), ...
...
function TransfMat = LocationTransform(Loc, TransfBefore, TransfAfter)
  ...
  %   Loc = TransfAfter(1:3, :) * TransfBefore * [[Loc(1:3), Loc(4:6), Loc(5:9)]; 1, 1, 1]; % test if efficiency difference.
