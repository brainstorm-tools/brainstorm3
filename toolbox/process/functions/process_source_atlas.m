    ...
    for iScout = 1:length(sScouts)
        ...
        if (ResultsMat.nComponents == 1)
            ...
            isBOLD = ~isempty(strfind(TestTags, 'bold')) || ...
                      ( isfield( ResultsMat, 'DisplayUnits') && strcmpi(ResultsMat.DisplayUnits, 'BOLD'));
                        
            isNIRS = ~isempty(strfind(TestTags, 'NIRS')) || ...
                     ~isempty(strfind(TestTags, 'Summed_sensitivities')) || ...
                     ( isfield( ResultsMat, 'DisplayUnits') && (~isempty(strfind(ResultsMat.DisplayUnits, 'mol.l-1')) || ~isempty(strfind(ResultsMat.DisplayUnits, 'OD'))));
