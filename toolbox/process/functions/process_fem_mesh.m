...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    % Zeffiro options: Max tetrahedral volume
    sProcess.options.zefMaxVol.Comment = 'Max tetrahedral volume (in cm<SUP>3</SUP>) TODO: ';
    sProcess.options.zefMaxVol.Type    = 'value';
    sProcess.options.zefMaxVol.Value   = {OPTIONS.ZefMaxVol, '', 4};
end
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    OPTIONS.ZefMaxVol = sProcess.options.zefMaxVol.Value{1};
    % sProcess.options.zefMaxVol.Value   = {OPTIONS.ZefMaxVol, '', 4};
...
function OPTIONS = GetDefaultOptions()
    OPTIONS = struct(...
        ...
        'zefMaxVol' ,     3, ...               % Zeffiro: Max tetrahedral volm...
        'zefKeepRatio',   100, ...          % iso2mesh: Percentage of elements kept (1-100%)
...
function [isOk, errMsg] = Compute(iSubject, iMris, isInteractive, OPTIONS)
    ...
    switch lower(OPTIONS.Method)
        ...
        case 'zeffiro'
            ...
            bst_plugin('SetProgressLogo', 'zeffiro'); % Todo add zeffiro logo
            ...
            %% Call Zeffiro ==> need help from Sampsa
            % move the working directory of Zef to temporary folder brainstorm2zef flder
            bst2zefInterface = bst_fullfile(PlugDesc.Path, PlugDesc.SubFolder, '+utilities', '+brainstorm2zef');
            % % Move the folder +brainstorm2zef folder to the tmp folder
            % movefile(bst2zefInterface, TmpDir);
...
function ComputeInteractive(iSubject, iMris, BemFiles) %#ok<DEFNU>
    ...
    switch (OPTIONS.Method)
        ...
        case 'zeffiro'
            ...
            [res, isCancel]  = java_dialog('input', {['Max tetrahedral volume (in cm^3) (10=coarse, 0.0001=fine):'], 'Percentage of elements kept (1-100%):'}, ...
                'Zeffiro FEM mesh', [], {num2str(OPTIONS.zefMaxVol), num2str(OPTIONS.zefKeepRatio)});
           
            if isCancel || isempty(str2double(res))
                return
            end
            % Get new values
            OPTIONS.zefMaxVol    = str2num(res{1});
            OPTIONS.KeepRatio = str2num(res{2}) ./ 100;
