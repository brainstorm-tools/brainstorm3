...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    % Zeffiro options: Mesh resolution [Add more option here? TODO]
    ...
    sProcess.options.zefUseGPU.Value   = 0;
end
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    % Zeffiro: Add here possible Zef Options
...
function OPTIONS = GetDefaultOptions()
    OPTIONS = struct(...
        ...
        'zefMeshResolution' ,     3, ...       % zefMeshResolution (old zefMaxVol): size of the esge...
        'zefUseGPU',   0, ...                  % zefUseGPU: Use the GPU for the mesh generation 
...
function [isOk, errMsg] = Compute(iSubject, iMris, isInteractive, OPTIONS)
    ...
    switch lower(OPTIONS.Method)
        ...
        case 'zeffiro'
            ...
            % If surfaces are not passed in input: get default surfaces
            % Zeffiro require inverse order ... from outer to inner: 
            % 
            if isempty(OPTIONS.BemFiles)
                if ~isempty(sSubject.iScalp) && ~isempty(sSubject.iOuterSkull) && ~isempty(sSubject.iInnerSkull)
                    OPTIONS.BemFiles = {...
                ...
                iSort = flip([iSort, iOther(~isnan(iOther))]); % flip will reverse the order of the surfaces
                OPTIONS.BemFiles = OPTIONS.BemFiles(iSort);
                TissueLabels = TissueLabels(iSort);
                % If there is a CSF layer but nothing inside: rename into BRAIN
                if ismember('csf', TissueLabels) && ~ismember('white', TissueLabels) && ~ismember('gray', TissueLabels)
            ...
            % General setting : TODO: ask Sampsa for a detailled explanation
            ...
            fprintf(fid, 'zef.exclude_box = %d;\n', 1); % include the bounding box in the mesh => default no ==>1
            fprintf(fid, 'zef.max_surface_face_count = %s;\n', num2str(0.5)); %  default is 0.5 (halp of the inputs), similar to keep ratio
            %   fit to the input surface => <1 ==> coraser mesh, if >1 it
            %   finer mesh at the surface. ==> 0.5 ==> advanced parameters
            %   ==> todo
            fprintf(fid, 'zef.mesh_smoothing_on = %d;\n', 1); % which part of the mesh to smooth?
            % this option smooth all the volum => which routine Taubin
            % method both the volum and strucutre. ==> To do from Zef side
            % for advanced users. 

            fprintf(fid, 'zef.mesh_resolution = %d;\n', OPTIONS.zefMeshResolution); 
            % what is the units (it is the unit of the input surface) in BST it is mm;
            % and what are the possible values interval => the coarsest value is 4.5mm
            % interval of possible value [minValue = 1.3  maxValue = 4.5]mm
            % https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0290715
            % IT MAY GO TO 1mm, Fernando will check that
            %==> it depends on the zef.ini parameters 
            % C:\Users\takfa\.brainstorm\plugins\zeffiro\zeffiro_interface-main_development_branch\profile\zeffiro_interface.ini
            % CHANGE THE LINE 6: 100 TO 20
            % Level of parallelization in mesh labeling,20,parallel_vectors,number
            % Level of parallelization in mesh labeling,20,parallel_vectors,number

            
            fprintf(fid, 'zef.use_gpu = %d;\n', OPTIONS.zefUseGPU); % what is the units and what are the possible values interval 
            % vecore that affect zef.ini  ==> 
            %in line 11: when GPU is off CPU is running, it uses 4 cpu per
            % default ==> advanced options 
            % Parallel threads in CPU forward computing,4,parallel_processes,number
                % \zeffiro_interface.ini
            % Refinement
            % edit zef_init_forward_and_inverse_options.m
            % edit zef_open_forward_and_inverse_options.m
            fprintf(fid, 'zef.refinement_on = %d;\n', 0); % boolean refinement of what?:
            % boolean value to refine the mesh ==> but what part of the
            % mesh ==> default 1

            fprintf(fid, 'zef.refinement_surface_on = %d;\n', 0); % Activat the refinement of the surface, default 1
            ...
            % what does it means => take the outer most from  end-%d:end-1
            % ==> end it s the bounding box, 
            % ==> list of the active compartment
            fprintf(fid, 'import_compartment_list_aux = import_compartment_list_aux(:)'';\n');
            fprintf(fid, 'zef.refinement_surface_compartments = [-1 import_compartment_list_aux];\n'); % what is this value mean
            % -1 mean ==> if LF is generated ==> it will refined
            % refine all the compartement that have the sources ==> in this
            % case we dont have ==> and iit will also refine the labeld
            % tissue in import_compartment_list_aux indexes
            
            % basic options ==> that can be used. 
            % and use the "advanced options for ":
            % utilities.brainstorm2zef.m.zef_bst_plugin_start(path)
            
            % import other mesh options
            ...
            for iBem = 1:nBem 
                fprintf(fid, ...TODO: need to discuss with Sampsa
            ...
            % TODO: how the data is passed to Zeffiro?
            %https://github.com/sampsapursiainen/zeffiro_interface/blob/master/%2Bexamples/zef_meshing_example.m
            % zef = zeffiro_interface('start_mode','no display','import_to_new_project',...
            %     './+utilities/+brainstorm2zef/BrainStorm2Zeffiro_import_2.zef',...
            %     'run_script','zef = zef_create_finite_element_mesh(zef)','exit_zeffiro;');
            ...
            % smoothing and 
            % read Zeff outputs TODO
            % conversion: figure; plotmesh(zef.nodes, [zef.tetra zef.domain_labels], 'x>0')
            node = zef.nodes;
