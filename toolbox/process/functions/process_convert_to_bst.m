...
function OutputFiles = Run(sProcess, sInputs, method) %#ok<DEFNU>
    ...
    for iInput = 1:length(sInputs)
        ...
        selectedChannels = sProcess.options.channels.Value{1}; % Selected channels to included in the converted file
        ...
        %% Convert the ChannelMat based on the channel selection
        a = strsplit(ChannelMatOut.Comment,'('); b = strsplit(a{2},')');
        ChannelMatOut.Comment = [a{1} '(' num2str(length(selectedChannels)) ')']; clear a b
        ChannelMatOut.Channel = ChannelMatOut.Channel(selectedChannels);
        
        
        sFileTemplate = sMat.F;
        
        sFileTemplate.channelflag      = sFileTemplate.channelflag(selectedChannels);
        sFileTemplate.header.nchannels = length(selectedChannels);
        ...
        [sFileOut, errMsg] = out_fopen(RawFileOut, RawFileFormat, sFileTemplate, ChannelMatOut);
        ...
        LFP = zeros(length(selectedChannels), length(downsample(sMat.Time,round(Fs/NewFreq)))); % This shouldn't create a memory problem
        bst_progress('start', 'Spike-sorting', 'Converting RAW signal to .bst file...', 0, (sProcess.options.paral.Value == 0) * nChannels);

        
        if sProcess.options.paral.Value
            parfor iChannel = selectedChannels
