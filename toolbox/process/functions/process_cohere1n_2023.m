...
function sProcess = GetDescription()
    ...
    % === Hilbert/Morlet
    sProcess.options.tfmeasure.Comment = {'Instantaneous (Hilbert)', 'Morlet wavelets', 'Spectral (Fourier)', ''; 'hilbert', 'morlet', 'fourier', ''};
...
%     sProcess.options.tfsplit.Class   = 'hilbert';
    ...
    sProcess.options.avgwinlength.Value   = {1, 's', 3};
...
function OutputFiles = Run(sProcess, sInputA)
    ...
    switch OPTIONS.tfMeasure
        case 'hilbert'
            ...
            if strcmpi(sProcess.options.time.Value, 'windowed')
        ...
        case 'fourier'
            ...
            if strcmpi(sProcess.options.time.Value, 'windowed')
                OPTIONS.nAvgWin = sProcess.options.avgwinnum.Value{1};
            end
    end
    % Keep time or not; now explicit flag, no longer separate method '...t'
    OPTIONS.isKeepTime = ismember(sProcess.options.time.Value, {'dynamic', 'windowed'});
