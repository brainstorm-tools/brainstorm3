...
%
% USAGE:  [preCoh, freq, nWin, nFFT, Messages] = bst_cross_spectrum(Xs, Ys, Fs, WinLen, Overlap=0.5, CohMeasure='mscohere', isSymmetric=0, ImagingKernel=[], waitMax=100)
%
% INPUTS:
%    - X      : Signals X [nSignalsX, nSamplesX]
%    - Y      : Signals Y [nSignalsY, nSamplesY]
...
% 
% OUTPUTS:
%    - preCoh : Structure containing Sxx, Syy and Sxy
%
...
% Signals X and Y must have same nSamples
nSamplesX = size(X, 2);
nSamplesY = size(Y, 2);
if ~isequal(nSamplesX, nSamplesY)
